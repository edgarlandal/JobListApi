...
@router.get("", response_model=List[UserReponse], dependencies=[Depends(require_role([UserRole.ADMIN]))])
...
@router.patch("/{user_id}", response_model=UserReponse, dependencies=[Depends(require_role([UserRole.ADMIN]))])
